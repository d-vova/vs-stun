...
var MappedAddress = module.exports = function MappedAddress ( message, object, padding ) {
  var type = MappedAddress.TYPE;
  this.name = MappedAddress.NAME;
...
MappedAddress.prototype = new Attribute();

MappedAddress.TYPE = 0x0001;
MappedAddress.NAME = 'mappedAddress';

MappedAddress.parse = function parse ( message, value, padding ) {
  if ( value.length < 4 ) {
  ...
  return new MappedAddress(message, object, padding);
}
